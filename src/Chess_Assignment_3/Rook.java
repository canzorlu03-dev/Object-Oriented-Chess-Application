...
public class Rook extends Piece {
    ...
    // Method to determine if the Rook can move to a new position
    @Override
    public boolean canMove(int newX, int newY) {
        ...
        // Check for moving straight along the same row to the right
        else if (newY == y && newX > x) {
            for (int i = (x + 1); i < newX; i++) {
                if (Board.board[i][newY] != null) {
                    return false; // Invalid move if there are pieces in the path
                }
            }
            return true; // Valid move if the path is clear

        } 
        // Check for moving straight along the same column upwards
        ...
        // Check for moving straight along the same row to the left
        else if (newY == y && newX < x) {
            for (int i = (x - 1); i > newX; i--) {
                if (Board.board[i][newY] != null) {
                    return false; // Invalid move if there are pieces in the path
        ...
        // Check for moving straight along the same column downwards
        else if (newX == x && newY < y) {
            for (int i = (x - 1); i > newX; i--) {
                if (Board.board[i][newY] != null) {
                    return false; // Invalid move if there are pieces in the path
